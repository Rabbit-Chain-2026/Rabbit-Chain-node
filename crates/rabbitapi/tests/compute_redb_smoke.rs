...
fn parse_result(resp: &rabbitapi::rpc::JsonRpcResponse) -> serde_json::Value {
...
async fn compute_submit_result_output_smoke_redb_backend() {
    ...
    let ts = SystemTime::now()
        ...
        .expect("time ok")
        .as_millis();
    path.push(format!("rabbitchain-redb-smoke-{}.db", ts));
    ...
    let query_resp = api
        .handle_request(JsonRpcRequest {
            jsonrpc: "2.0".to_string(),
            method: "rabbit_getComputeTxResult".to_string(),
